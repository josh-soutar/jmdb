...
export default function Header() {
    ...
    function handleMouseEnter(event) {
        ...
        console.log(`${event.target.id}  width: ${event.target.offsetWidth} offset: ${event.target.offsetLeft}`) //Sometimes the ID isn't getting passed. This causes width and offset values to be incorrect        
    ...
    let menuElements = menuItems.map(item => (
        <MenuItem key={item.id} id={item.id} forwardedAs="div" pr="10px" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={{ opacity: item.fade_text ? 0.5 : 1 }}>
            <Link to={"/" + item.page_id} activeStyle={activeLinkStyle}>{item.title}</Link>
        </MenuItem>
    ))
    ...
    return (
        <HeaderContainer>
            ...
            <HeaderMenuContainer className="HeaderMenuContainer" display="flex" onMouseEnter={() => { setDisplayHoverBox(true) }} onMouseLeave={() => { setDisplayHoverBox(false) }}>
                <MenuItemOutline className="MenuItemOutline" style={{ opacity: displayHoverBox && "1", left: menuOutlineOffset, width: menuOutlineWidth }} />
                {menuElements}
            </HeaderMenuContainer>
